...
/** Full card: everything, including custom fields and every listing. */
export function ProductDetail({
  ...
  const listings = product
    ? [...product.listings].sort((a, b) => (a.priceCents ?? Infinity) - (b.priceCents ?? Infinity))
    : [];
    ...
    <Sheet open={!!product} onClose={onClose} label={product?.name ?? "Product"}>
      {product && (
        <div className="grid gap-6 p-4 sm:grid-cols-2 sm:p-6">
          ...
          <motion.div variants={staggerGrid} initial="hidden" animate="show" className="flex flex-col">
            ...
            <motion.p variants={riseIn} className="mt-2 text-2xl font-semibold">
            ...
            <motion.div variants={riseIn} className="mt-6">
              <h3 className="text-sm font-medium text-muted">
                {listings.length > 1 ? `Available in ${listings.length} sheets` : "Available from"}
              </h3>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-muted">
                        ...
                        {i === 0 && listings.length > 1 && <span className="ml-2 text-success">Lowest</span>}
                      </p>
                    </div>
                    <a
                      href={l.link}
