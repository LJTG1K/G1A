-- Storefront queries. Listings from the given (source, mapping signature) pairs are
-- merged by marketplace item into products; the cheapest listing supplies name and image.
-- SECURITY INVOKER: listings are publicly readable, so RLS already allows this.
...
-- Counts for the filter chips, over merged products.
...
language sql stable security invoker set search_path = '' as $$
  ...
  l as (
    select l.source_id, l.price_cents, l.category,
      l.item_key || '#' || row_number() over (partition by l.source_id, l.signature, l.item_key order by l.row_index) as item_key
    from public.listings l
    join src on l.source_id = src.source_id and l.signature = src.signature
  ),
  g as (
...
grant execute on function public.store_products(uuid[], text[], text[], text, uuid, integer, integer, text, integer, integer) to anon, authenticated;
grant execute on function public.store_facets(uuid[], text[]) to anon, authenticated;
